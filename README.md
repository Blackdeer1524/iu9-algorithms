Конфигурирование задачки

`cmake -GNinja -DCUR_MODULE=<один из трех модулей> -DQUESTION=<задачка из модуля> -DTARGET_GROUP=<production/test> -B ./build`

`TARGET_GROUP` - подключение локальных тестов

# Префиксы перед задачкой

* *нет префикса* - не сделано
* `[x]`  - задачка сделана, решение прошло тестовую систему, приняли;
* `[?]`   - задачка сделана, решение прошло тестовую систему, **НЕ** приняли, так как нашли слишком похожее решение;
* `[*]`   - решение **НЕ** прошло тестовую систему;
* `[...]` - задачка сделана, решение прошло тестовую систему, преподаватель пока не проверил.
...
### [x] 1.2  Произведение чисел по модулю (`mulmod.c`)
...
Сортировать массивы запрещается.
 ...
 ### [x] 1.6 Максимальная сумма подряд идущих элементов массива (`maxk.c`)
...
### [x] 1.8 Седловая точка в матрице (`saddlepoint.c`)
...
### [x] 1.17 Рисование рамки (`frame.c`)
...
### [x] 2.1 Кратчайшая суперстрока (`superstr.c`)
...
### [x] 2.5 Сортировка подсчётом сравнений (`csort.c`)
...
### [x] 2.9 Сортировка букв в строке (`dsort.c`)
``
...
### [x] 2.10 Поразрядная сортировка дат (`datesort.c`)
...
### [x] 2.11 Поразрядная сортировка целых чисел (`radixsort.c`)
...
### [x] 2.12 Периодические префиксы (`prefixes.c`)
...
### [x] 2.13 Поиск всех вхождений подстроки в строку (Кнут–Моррис–Пратт) (`kmpall.c`)
...
### [x] 2.14 Слово, составленное из префиксов другого слова (`pword.c`)
...
### [x] 2.15 Поиск всех вхождений подстроки в строку (Бойер–Мур) (`bmall.c`)
...
### [x] 2.16 Расширенная эвристика стоп-символа (`extstop.c`)
...
### [x] 2.17 Поиск максимального элемента подпоследовательности (`rangemax.c`)
...
### [x] 2.18 Определение гипердромов в строке (`rangehd.c`)
...
### [x] 2.21 Максимальное произведение простых дробей (`maxprod.c`)
...
### [x] 3.1 Нерекурсивная быстрая сортировка (`qsstack.c`)
...
### [x] 3.2 Стековая машина (`stackmachine.c`)
...
### [x] 3.3 Кольцевой буфер (`circbuf.c`)
...
### [x] 3.4 Очередь с операцией Maximum (`qmax.c`)
...
### [x] 3.5 Слияние последовательностей (`merge.c`)
...
### [x] 3.6 Моделирование работы вычислительного кластера (`cluster.c`)
...
### [x] 3.7 Сортировка списка вставками (`listisort.c`)
...
### [x] 3.8 Сортировка списка пузырьком (`listbsort.c`)
...
### [x] 3.9 Ранги элементов в списке с пропусками (`ranklist.c`)
